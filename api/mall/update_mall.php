...
if (
   ...
   // && isset($_POST["mal_not"])
    ...
    $updateArray = array();
	array_push($updateArray, htmlspecialchars(strip_tags($mal_name)));
    array_push($updateArray, htmlspecialchars(strip_tags($mal_not)));
	...
	if($img_image != "")
	{
		$sql = "update mall set mal_name=?,mal_not=?,cit_id=?,str_id=?,
		mal_image = ? , mal_thumbnail = ? 
		where mal_id=?";
	}
	else
	{
