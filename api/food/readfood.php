...
if (
	...
	if(trim($txtsearch) != "")
	{
		$sql = "select * from food where  cat_id = ? and foo_name like ? order by foo_id asc limit $start , $end";
		$result = dbExec($sql, $selectArray);
	}
	else
	{
		$sql = "select * from food where  cat_id = $cat_id order by foo_id asc limit $start , $end";
		$result = dbExec($sql, []);
	}
    $arrJson = array();
