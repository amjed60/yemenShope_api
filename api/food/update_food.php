...
if (
	...
	array_push($updateArray, htmlspecialchars(strip_tags($foo_barcode)));
	...
	if($img_image != "")
	{
		...
		foo_image = ? , foo_thumbnail = ? ,foo_barcode= ?
		where foo_id=?";
	}
	else
	{
		...
		set foo_name=?,foo_name_en=?,
		set foo_price=?,foo_offer=?,
		set foo_info=?,foo_info_en=?,foo_barcode= ?
