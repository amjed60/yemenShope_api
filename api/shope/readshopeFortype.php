...
if (
	...
	if(trim($txtsearch) != "")
	{
		$sql = "select * from shope where  typ_id = ? and sho_name like ? order by sho_name asc";
		$result = dbExec($sql, $selectArray);
	}
  
	else
	{
		$sql = "select * from shope where typ_id = $typ_id order by sho_name asc limit $start , $end";
		$result = dbExec($sql, []);
	}// and isactive!=1
    $arrJson = array();
