...
if (
    ...
    array_push($insertArray, htmlspecialchars(strip_tags($pro_id)));
	
	  

    $sql = "insert into favorite
        (cus_id , sho_id ,pro_id, fav_regdate )
            values(? , ? ,?,  now())";
    $result = dbExec($sql, $insertArray);

	  $readArray = array();
	
    array_push($readArray, htmlspecialchars(strip_tags($sho_id)));
	array_push($readArray, htmlspecialchars(strip_tags($cus_id)));
	
    $sql = "select * from favorite where cus_id = ? and sho_id = ?  order by sho_id desc limit 0,1";
