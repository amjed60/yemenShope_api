...
//	&& is_numeric($_GET["sho_id"])
	...
	if(trim($txtsearch) != "")
	{
		$sql = "select * from product where  sho_id = ? and pro_name like ? order by pro_id asc limit $start , $end";
		$result = dbExec($sql, $selectArray);
	}
	else
	{
		$sql = "select * from product where  sho_id = $sho_id order by pro_id asc limit $start , $end";
		$result = dbExec($sql, []);
	}
    $arrJson = array();
